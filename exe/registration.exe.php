...
if($selectbio[0]["id"]>""){
	$sqlupdatebio = "update bio set 
		nome_band = '".strip_tags(addslashes(stripslashes($_REQUEST['nome_band'])))."',
		bio = '".strip_tags(addslashes(stripslashes($_REQUEST['biografia'])))."',
		message = '".strip_tags(addslashes(stripslashes($_REQUEST['messaggio'])))."',
		video =' ".strip_tags(addslashes(stripslashes($_REQUEST['video_url'])))."'
	...
	$sqlinsertbio = "insert into bio (id_user,nome_band,bio,message,video,image) values (
		...
		'".strip_tags(addslashes(stripslashes($_REQUEST['nome_band'])))."',
		'".strip_tags(addslashes(stripslashes($_REQUEST['biografia'])))."',
		'".strip_tags(addslashes(stripslashes($_REQUEST['messaggio'])))."',
...
if($_FILES["canzone"]["size"]>0 && VOTE_SESSION == false){
	...
	//in_array($_FILES['canzone']['type'], $mimes) && $_FILES["canzone"]["size"] < 8000000
	if (true){
...
header("Location: http://".$_SERVER['HTTP_HOST']."/registration.php?alert=".urlencode($alert));
ob_end_flush();
?>
	<?
